...
const statusColors = {
    ...
    'Fulfilled':'bg-purple-100 text-purple-700','Delivered':'bg-green-100 text-green-700',
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
                <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500 text-left">
                        <tr>{['Order #','Customer','Shop','Total','Status','Payment','Date','Action'].map(h => <th key={h} className="px-4 py-3 font-medium">{h}</th>)}</tr>
                    </thead>
                    <tbody className="divide-y">
                        {filtered?.map(o => (
                            <tr key={o.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 text-gray-500">{o.shop}</td>
                                ...
                                <td className="px-4 py-3"><Link href={`/admin/orders/${o.id}`} className="text-indigo-600 hover:underline text-xs">View</Link></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
