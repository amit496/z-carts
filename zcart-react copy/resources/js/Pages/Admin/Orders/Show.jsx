...
const orderStatuses = {1:'Pending',2:'Payment Error',3:'Confirmed',4:'Fulfilled',5:'Awaiting Delivery',6:'Delivered',7:'Returned',8:'Cancelled',9:'Disputed'};
const paymentStatuses = {1:'Unpaid',2:'Pending',3:'Paid',4:'Refund Initiated',5:'Partially Refunded',6:'Refunded'};

export default function OrderShow({ order }) {
    return (
        <AdminLayout title={`Order #${order.order_number}`}>
            <div className="flex items-center gap-3 mb-6">
                <Link href="/admin/orders" className="text-gray-400 hover:text-gray-600 text-sm">← Back to Orders</Link>
            </div>
                    ...
                    <div className="bg-white rounded-xl shadow p-5">
                        ...
                        <table className="w-full text-sm">
                            <thead><tr className="text-left text-gray-400 border-b">
                                <th className="pb-2">Product</th><th className="pb-2">Qty</th><th className="pb-2">Unit Price</th><th className="pb-2">Total</th>
                            </tr></thead>
                            <tbody>
                ...
                <div className="space-y-4">
                    <div className="bg-white rounded-xl shadow p-5">
                        ...
                        <div className="space-y-2 text-sm text-gray-600">
                            ...
                            <div className="flex justify-between"><span>Shop</span><span className="font-medium">{order.shop?.name}</span></div>
                            ...
                            <div className="flex justify-between border-t pt-2 font-bold text-gray-800"><span>Grand Total</span><span className="text-indigo-600">${order.grand_total}</span></div>
                        </div>
                    </div>

                    <div className="bg-white rounded-xl shadow p-5">
                        <h2 className="font-semibold text-gray-700 mb-3">Update Order Status</h2>
                        <select defaultValue={order.order_status_id}
                            onChange={e => router.patch(`/admin/orders/${order.id}/status`, { order_status_id: e.target.value })}
                    ...
                    <div className="bg-white rounded-xl shadow p-5">
                        <h2 className="font-semibold text-gray-700 mb-3">Update Payment Status</h2>
                        <select defaultValue={order.payment_status}
                            onChange={e => router.patch(`/admin/orders/${order.id}/payment-status`, { payment_status: e.target.value })}
                            className="w-full border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
                            {Object.entries(paymentStatuses).map(([k,v]) => <option key={k} value={k}>{v}</option>)}
                        </select>
                    </div>
                </div>
            </div>
