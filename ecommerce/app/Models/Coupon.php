...
class Coupon extends BaseModel
{
    ...
    /**
     * Simple JSON / Inertia coupon check (active + date window + quantity).
     */
    public function isValid(): bool
    {
        ...
        $now = Carbon::now();
        if ($this->starting_time && $now->lt($this->starting_time)) {
            return false;
        }
        if ($this->ending_time && $now->gt($this->ending_time)) {
            return false;
        }
        if ($this->quantity !== null && (int) $this->quantity <= 0) {
            return false;
        }
    ...
    public function calculateDiscount(float $subtotal): float
    {
        ...
        $min = (float) ($this->min_order_amount ?? 0);
        if ($subtotal < $min) {
            return 0;
        }
