...
export default function DeliveryBoysIndex({ deliveryBoys }) {
    const [search, setSearch] = useState('');
    const filtered = deliveryBoys.data?.filter(d => d.name?.toLowerCase().includes(search.toLowerCase()) || d.email?.toLowerCase().includes(search.toLowerCase()));
    return (
        <AdminLayout title="Delivery Boys">
            <PageHeader title="Delivery Boys" action={
                <Link href="/admin/delivery-boys/create" className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700">+ Add</Link>
            } />
            <div className="mb-4">
                        ...
                        {filtered?.map(d => (
                            <tr key={d.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 flex gap-3">
                                    <Link href={`/admin/delivery-boys/${d.id}/edit`} className="text-indigo-600 hover:underline text-xs">Edit</Link>
                                    <button onClick={() => { if (confirm('Delete?')) router.delete(`/admin/delivery-boys/${d.id}`); }} className="text-red-500 hover:underline text-xs">Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
